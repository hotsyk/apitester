...
class ModBaseModelBackend(ModelBackend):

    create_unknown_user = True

    def authenticate(self, public_key=None, secret_key=None, token=None):
        client = ModDefaultClient(public_key, secret_key, token)
        try:
            api_token, auth_user = client.auth.check_token() 
        except HTTPError:
            return None
...
class ModModelBackend(ModBaseModelBackend):
